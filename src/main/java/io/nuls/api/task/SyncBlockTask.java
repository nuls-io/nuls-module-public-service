...
public class SyncBlockTask implements Runnable {
    ...
    //Record the number of synchronization errors
    ...
    private void process() {
        ...
        //Before synchronizing data, check the latest synchronization information. If the latest block of data is not fully processed in a single transaction, it is necessary to roll back the block data
     ...
     * <p>
     * 1. Retrieve the latest saved block record locally
     * 2. Synchronize the next block of the wallet based on the height of the latest local block（If not available locally, proceed from the0Block Start）
     * 3. After synchronizing to the latest block, the task ends and waits for the next one10Seconds, resynchronize
     * 4. Each synchronization requires continuity verification with the previous block. If the verification fails, it indicates a local fork and requires rollback processing
     *
     * @return boolean Do you want to continue synchronizing
     */
